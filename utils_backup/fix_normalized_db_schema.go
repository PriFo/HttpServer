...
func main() {
	// Исправляем normalized_data.db
	db, err := sql.Open("sqlite3", "normalized_data.db")
	if err != nil {
		log.Fatalf("Ошибка открытия normalized_data.db: %v", err)
	}
	defer db.Close()

	fmt.Println("Обновление схемы normalized_data.db...")
	...
	fmt.Println("\n✓ Схема normalized_data.db обновлена!")
}
