    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/quality/violations/${violationId}`,
        {
          ...
          body: JSON.stringify({
            // NOTE: Will be replaced with actual user from auth context when authentication is implemented
            // В данный момент используется заглушка 'User'
            resolved_by: 'User'
          })
        }
      )
...
export default function ViolationsPage() {
  return (
    <Suspense fallback={<div className="container mx-auto p-6">Загрузка...</div>}>
