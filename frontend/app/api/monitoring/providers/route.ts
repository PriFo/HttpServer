...
// Fallback данные для провайдеров мониторинга
...
export async function GET() {
  try {
    const data = await fetchJsonServer(`${BACKEND_URL}/api/monitoring/providers`, {
    ...
    const errorMessage = getServerErrorMessage(error, 'Не удалось получить метрики провайдеров')
