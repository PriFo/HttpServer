...
export async function POST(request: NextRequest) {
  try {
    ...
    if (finalClientId && finalProjectId) {
      ...
      const timeoutId = setTimeout(() => controller.abort(), 10000) // Увеличено до 10 секунд для больших проектов
    ...
    console.error('Error starting counterparties normalization:', error)
    return NextResponse.json(
      {
