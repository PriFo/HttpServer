...
func main() {
	...
	fmt.Println("=== Бенчмарк всех моделей Hugging Face API ===")
	fmt.Printf("Сервер: %s\n", serverURL)
	fmt.Println()
	...
	if err != nil {
		...
		fmt.Printf("⚠ Убедитесь, что сервер запущен на %s\n", serverURL)
		fmt.Println("⚠ Будет использован fallback список моделей")
		fmt.Println()
	} else {
		resp.Body.Close()
	...
	// Получаем список моделей из Hugging Face API
	...
	fmt.Println("Все модели Hugging Face Inference API бесплатные (с ограничениями по rate limit)")
	fmt.Println()
	for i, model := range models {
		fmt.Printf("  %d. %s (бесплатная)\n", i+1, model)
	}
	fmt.Println()
	...
	fmt.Println("ИНФОРМАЦИОННАЯ ПАНЕЛЬ: БЕНЧМАРК МОДЕЛЕЙ HUGGING FACE API")
	fmt.Println("=" + repeat("=", 100))
	fmt.Println()
	...
	if len(sortedModels) > 0 {
		...
		if fastest != nil {
			...
			fmt.Printf("  HUGGINGFACE_MODEL=%s\n", fastest.Name)
			fmt.Printf("  MaxWorkers=1 (для Hugging Face)\n")
			fmt.Printf("  RateLimit=30 (30 запросов/мин)\n")
		} else {
			fmt.Println("⚠ ВНИМАНИЕ: Нет моделей с успешными запросами!")
			fmt.Println("  Возможные причины:")
			fmt.Println("  1. HUGGINGFACE_API_KEY не установлен")
			fmt.Println("  2. Модели недоступны в Hugging Face API")
			fmt.Println("  3. Проблемы с сетью или API сервисом")
			fmt.Println()
			fmt.Println("  Для полного тестирования установите HUGGINGFACE_API_KEY и перезапустите сервер")
		}
	}
...
func saveResultsToJSON(benchmarks []*ModelBenchmark) {
	...
	if err == nil {
		filename := fmt.Sprintf("huggingface_models_benchmark_%s.json", time.Now().Format("20060102_150405"))
...
<head>
    <meta charset="UTF-8">
    <title>Бенчмарк моделей Hugging Face API</title>
    <style>
...
<body>
    <div class="container">
        <h1>🏆 Бенчмарк моделей Hugging Face API</h1>
...
`, fastest.Name, fastest.Speed, fastest.AvgResponseTime.Round(time.Millisecond),
	...
	filename := fmt.Sprintf("huggingface_models_benchmark_%s.html", time.Now().Format("20060102_150405"))
...
func getAvailableModels(serverURL string) ([]string, error) {
	// Получаем модели через API сервера с параметром provider=huggingface
	resp, err := http.Get(serverURL + "/api/workers/models?provider=huggingface")
	if err == nil && resp.StatusCode == http.StatusOK {
		...
		if err := json.NewDecoder(resp.Body).Decode(&apiResp); err == nil {
			...
			if apiResp.Success && len(apiResp.Data.Models) > 0 {
				...
				if len(models) > 0 {
					fmt.Printf("Получено %d моделей из API\n", len(models))
					return models, nil
				}
			}
		}
	...
	// Используем известные модели Hugging Face как fallback
	knownModels := []string{
	...
	fmt.Printf("Используются fallback модели: %v\n", knownModels)
...
func testModel(serverURL, modelName string, testProducts []string) *ModelBenchmark {
	...
	for _, product := range testProducts {
		...
		// Обновляем минимальное и максимальное время ответа
		if benchmark.MinResponseTime == time.Hour || reqDuration < benchmark.MinResponseTime {
	...
	// Рассчитываем среднее время для всех запросов (включая ошибки)
	...
	if benchmark.SuccessCount > 0 && totalTime.Seconds() > 0 {
		...
		if benchmark.SuccessCount > 0 {
			// Используем только времена успешных запросов
			var successDuration int64
			for _, rt := range benchmark.ResponseTimes {
				successDuration += int64(rt)
			}
			if successDuration > 0 {
				benchmark.AvgResponseTime = time.Duration(successDuration) / time.Duration(benchmark.SuccessCount)
			}
		}
