...
func (s *Server) handleResetClassification(w http.ResponseWriter, r *http.Request) {
	...
	if req.ResetAll {
		...
		s.writeJSONError(w, r, "Не указаны критерии для сброса. Укажите normalized_name, category, kpved_code или установите reset_all=true", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		...
		s.writeJSONError(w, r, fmt.Sprintf("Failed to reset classification: %v", err), http.StatusInternalServerError)
		return
	}
...
func (s *Server) handleResetAllClassification(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		s.writeJSONError(w, r, fmt.Sprintf("Failed to reset all classifications: %v", err), http.StatusInternalServerError)
		return
	}
...
func (s *Server) handleResetByCode(w http.ResponseWriter, r *http.Request) {
	...
	if req.KpvedCode == "" {
		s.writeJSONError(w, r, "kpved_code is required", http.StatusBadRequest)
		return
	}
...
func (s *Server) handleResetLowConfidence(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		s.writeJSONError(w, r, fmt.Sprintf("Failed to reset low confidence classifications: %v", err), http.StatusInternalServerError)
		return
	}
...
func (s *Server) handleMarkIncorrect(w http.ResponseWriter, r *http.Request) {
	...
	if req.NormalizedName == "" || req.Category == "" {
		s.writeJSONError(w, r, "normalized_name and category are required", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		...
		s.writeJSONError(w, r, fmt.Sprintf("Failed to mark as incorrect: %v", err), http.StatusInternalServerError)
		return
	}
...
func (s *Server) handleMarkCorrect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		s.writeJSONError(w, r, fmt.Sprintf("Failed to mark as correct: %v", err), http.StatusInternalServerError)
		return
	}
...
func (s *Server) handleKpvedWorkersStatus(w http.ResponseWriter, r *http.Request) {
	...
	s.writeJSONResponse(w, r, status, http.StatusOK)
}
...
func (s *Server) handleKpvedStatsIncorrect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		s.writeJSONError(w, r, fmt.Sprintf("Failed to get incorrect classifications: %v", err), http.StatusInternalServerError)
		return
	}
	defer rows.Close()
