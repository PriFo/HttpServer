...
// Использует PatternDetectionHandler
func (s *Server) handlePatternDetect(w http.ResponseWriter, r *http.Request) {
	if s.patternDetectionHandler != nil {
		s.patternDetectionHandler.HandleDetectPatterns(w, r)
		return
	}
	// Fallback если handler не инициализирован
	s.writeJSONError(w, r, "Pattern detection handler not initialized", http.StatusServiceUnavailable)
}
...
func (s *Server) handlePatternTestBatch(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeJSONError(w, r, fmt.Sprintf("Database query error: %v", err), http.StatusInternalServerError)
		return
	}
	defer rows.Close()
