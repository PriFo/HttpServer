...
Получает сводную информацию по всем базам данных системы. Сканирует все загрузки, подсчитывает общую статистику и формирует отчет о состоянии системы.
...
**Описание работы:**
1. Подключается к `service.db` и основной БД (`data.db`)
2. Получает список всех загрузок из основной БД
3. Для каждой загрузки находит путь к файлу БД через `database_id` → `project_databases`
4. Открывает файлы БД параллельно (до 5 одновременно) и подсчитывает записи в таблицах `nomenclature_items` и `counterparties`
5. Агрегирует общую статистику и формирует отчет
...
```json
{
  "total_databases": 15,
  "total_uploads": 25,
  "completed_uploads": 22,
  "failed_uploads": 2,
  "in_progress_uploads": 1,
  "last_activity": "2025-01-21T18:30:00Z",
  "total_nomenclature": 1523456,
  "total_counterparties": 98765,
  "upload_details": [
    {
      "id": "1",
      "upload_uuid": "9d09249b-50c7-4a7a-ae76-2220af689f99",
      "name": "БухгалтерияДляКазахстана",
      "status": "completed",
      "created_at": "2025-01-20T15:37:48Z",
      "completed_at": "2025-01-20T15:42:34Z",
      "nomenclature_count": 16065,
      "counterparty_count": 2301,
      "database_file": "data/uploads/9d09249b-50c7-4a7a-ae76-2220af689f99.db",
      "database_id": 5,
      "client_id": 1,
      "project_id": 2
    }
...
**Поля ответа:**
- `total_databases` (number) - общее количество уникальных баз данных
- `total_uploads` (number) - общее количество загрузок
- `completed_uploads` (number) - количество завершенных загрузок
- `failed_uploads` (number) - количество проваленных загрузок
- `in_progress_uploads` (number) - количество загрузок в процессе
- `last_activity` (string, ISO 8601) - дата и время последней активности
- `total_nomenclature` (number) - общее количество записей номенклатуры во всех БД
- `total_counterparties` (number) - общее количество контрагентов во всех БД
- `upload_details` (array) - детальная информация по каждой загрузке
  - `id` (string) - ID загрузки
  - `upload_uuid` (string) - UUID загрузки
  - `name` (string) - имя конфигурации 1С
  - `status` (string) - статус: `completed`, `failed`, `in_progress`
  - `created_at` (string, ISO 8601) - время создания загрузки
  - `completed_at` (string, ISO 8601, optional) - время завершения загрузки
  - `nomenclature_count` (number) - количество номенклатуры в БД загрузки
  - `counterparty_count` (number) - количество контрагентов в БД загрузки
  - `database_file` (string) - путь к файлу базы данных
  - `database_id` (number, optional) - ID базы данных в service.db
  - `client_id` (number, optional) - ID клиента
  - `project_id` (number, optional) - ID проекта

**Коды ответа:**
- `200 OK` - успешный запрос
- `405 Method Not Allowed` - неподдерживаемый HTTP метод
- `500 Internal Server Error` - ошибка при сканировании систем

**Пример использования:**

```bash
curl -X GET http://localhost:9999/api/system/summary
```

**Особенности:**
- Функция сканирует все базы данных параллельно (до 5 одновременно)
- Если база данных не найдена или не открывается, загрузка все равно включается в отчет, но счетчики будут равны 0
- Поддерживает различные варианты таблиц для контрагентов: `counterparties`, `normalized_data`, `catalog_items`
- Использует кеширование путей к БД для оптимизации производительности
- Таймаут на каждую БД: 5 секунд
- Результаты кешируются на 2 минуты для ускорения повторных запросов

---

#### GET /api/system/summary/cache/stats

Получает статистику кеша системной сводки. Позволяет отслеживать эффективность кеширования и состояние кеша.
...
```json
{
  ...
  "has_data": true,
  "is_stale": false,
  "expiry": "2025-11-21T18:32:00Z",
  "time_to_expiry": "1m23s"
}
```

**Поля ответа:**
- `hits` (number) - количество попаданий в кеш (успешных обращений)
- `misses` (number) - количество промахов кеша (когда данных нет или они устарели)
- `hit_rate` (number, 0-1) - процент попаданий в кеш (hits / (hits + misses))
- `has_data` (boolean) - есть ли данные в кеше
- `is_stale` (boolean) - устарели ли данные в кеше
- `expiry` (string, ISO 8601) - время истечения срока действия кеша
- `time_to_expiry` (string) - оставшееся время до истечения кеша (например, "1m23s")

**Коды ответа:**
- `200 OK` - успешный запрос
- `405 Method Not Allowed` - неподдерживаемый HTTP метод
- `503 Service Unavailable` - кеш не инициализирован

**Пример использования:**

```bash
curl -X GET http://localhost:9999/api/system/summary/cache/stats
```
...
Инвалидирует (помечает как устаревший) кеш системной сводки. После этого следующий запрос к `/api/system/summary` выполнит новое сканирование системы. Устаревшие данные остаются в кеше как fallback на случай ошибок.
...
**Коды ответа:**
- `200 OK` - кеш успешно инвалидирован
- `405 Method Not Allowed` - неподдерживаемый HTTP метод
- `503 Service Unavailable` - кеш не инициализирован
...
```bash
curl -X POST http://localhost:9999/api/system/summary/cache/invalidate
```
...
Полностью очищает кеш системной сводки. Все данные удаляются из кеша. Полезно для полного сброса кеша или при изменении конфигурации системы.
...
```bash
curl -X POST http://localhost:9999/api/system/summary/cache/clear
```

**Примечание:** После очистки кеша следующий запрос к `/api/system/summary` обязательно выполнит полное сканирование системы.
...
> Примечание: Поле `alternatives` всегда присутствует в ответе и может содержать пустой массив, если альтернативные цепочки не найдены.
