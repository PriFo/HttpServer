...
type CatalogItemRequest struct {
	...
	Name        string   `xml:"name"`
	Attributes  string   `xml:"attributes"`  // JSON строка
	TableParts  string   `xml:"table_parts"` // JSON строка
	Timestamp   string   `xml:"timestamp"`
}
...
type ActivityLog struct {
	ID        int       `json:"id"`
	Type      string    `json:"type"` // "normalization", "upload", "error", etc.
	Message   string    `json:"message"`
	Timestamp time.Time `json:"timestamp"`
...
// DashboardOverviewResponse агрегированный ответ для главной страницы дашборда
type DashboardOverviewResponse struct {
	...
	ProviderMetrics     interface{}         `json:"provider_metrics,omitempty"` // MonitoringData, но без прямого импорта
}
...
// ClientDocument модель документа клиента
