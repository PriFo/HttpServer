...
// SystemSummary интерфейс для системной сводки (определен в server пакете)
// Используем interface{} чтобы избежать циклической зависимости
type SystemSummary interface{}

// SystemSummaryCache кэш для результатов сканирования системы
// Использует interface{} для SystemSummary, чтобы избежать циклической зависимости с server пакетом
type SystemSummaryCache struct {
	mu       sync.RWMutex
	summary  interface{} // *server.SystemSummary
	expiry   time.Time
	ttl      time.Duration
...
func (c *SystemSummaryCache) Get() (interface{}, bool) {
	c.mu.RLock()
...
func (c *SystemSummaryCache) Set(summary interface{}) {
	c.mu.Lock()
	defer c.mu.Unlock()
