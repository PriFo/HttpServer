...
func (wcm *WorkerConfigManager) initDefaultConfig() {
	...
	arliaiConfig := &ProviderConfig{
		...
		Enabled:    true,
		Priority:   1,
		MaxWorkers: 2,   // Ограничение на параллельные запросы, НЕ на количество моделей
	...
	// Дефолтная конфигурация для OpenRouter
	openrouterAPIKey := os.Getenv("OPENROUTER_API_KEY")
	openrouterConfig := &ProviderConfig{
		Name:       "openrouter",
		APIKey:     openrouterAPIKey,
		BaseURL:    "https://openrouter.ai/api/v1",
		Enabled:    openrouterAPIKey != "", // Включаем только если есть API ключ
		Priority:   1,                      // Высокий приоритет для OpenRouter
		MaxWorkers: 1,
		RateLimit:  60,
		Timeout:    60 * time.Second,
		Models: []ModelConfig{
			{
				...
				Provider:    "openrouter",
				Enabled:     true,
				Priority:    1, // Наивысший приоритет для z.ai/glm-4.5
				MaxTokens:   8192,
				Temperature: 0.3,
				Speed:       "medium",
				Quality:     "high",
	...
	wcm.providers["openrouter"] = openrouterConfig

	// Дефолтная конфигурация для Hugging Face
	huggingfaceAPIKey := os.Getenv("HUGGINGFACE_API_KEY")
	huggingfaceConfig := &ProviderConfig{
		Name:       "huggingface",
		...
		Priority:   3,
		MaxWorkers: 1,
		RateLimit:  30, // 30 запросов в минуту
		Timeout:    60 * time.Second,
		Models: []ModelConfig{
			{
	...
	wcm.providers["huggingface"] = huggingfaceConfig
...
// GetGlobalMaxWorkers возвращает глобальный максимум воркеров
func (wcm *WorkerConfigManager) GetGlobalMaxWorkers() int {
	wcm.mu.RLock()
	defer wcm.mu.RUnlock()
	return wcm.globalMaxWorkers
}
...
func (wcm *WorkerConfigManager) CreateAIClient() (*nomenclature.AIClient, error) {
	...
	if apiKey == "" {
		// Пытаемся получить из переменной окружения в зависимости от провайдера
		...
		default:
			// Fallback на ARLIAI_API_KEY для обратной совместимости
			apiKey = os.Getenv("ARLIAI_API_KEY")
		}
		if apiKey == "" {
			envVar := fmt.Sprintf("%s_API_KEY", strings.ToUpper(provider.Name))
			return nil, fmt.Errorf("API key not set for provider %s and %s environment variable not set", provider.Name, envVar)
		}
	}
...
func (wcm *WorkerConfigManager) GetModelAndAPIKey() (apiKey string, modelName string, err error) {
	...
	if apiKey == "" {
		...
		if apiKey == "" {
			envVar := fmt.Sprintf("%s_API_KEY", strings.ToUpper(provider.Name))
			return "", "", fmt.Errorf("API key not set for provider %s and %s environment variable not set", provider.Name, envVar)
		}
	}
...
func (wcm *WorkerConfigManager) GetNomenclatureConfig() (nomenclature.Config, error) {
	...
	if apiKey == "" {
		...
		if apiKey == "" {
			envVar := fmt.Sprintf("%s_API_KEY", strings.ToUpper(provider.Name))
			return nomenclature.Config{}, fmt.Errorf("API key not set for provider %s and %s environment variable not set", provider.Name, envVar)
		}
	}
	...
	// Rate limiting теперь контролируется через rate limiter в AIClient
