...
// Получение имени компьютера (безопасная версия для серверного контекста)
&НаСервере
Функция ПолучитьИмяКомпьютера() Экспорт
	
	Попытка
		// Используем глобальную функцию 1С
		Информация = ИнформацияОКомпьютере();
		Если Информация <> Неопределено Тогда
			Возврат Строка(Информация.ИмяКомпьютера);
		КонецЕсли;
	Исключение
		// Если функция недоступна, возвращаем пустую строку
	КонецПопытки;
...
// Получение имени пользователя (безопасная версия для серверного контекста)
&НаСервере
Функция ПолучитьИмяПользователя() Экспорт
	
	Попытка
		// Используем глобальную функцию 1С
		Информация = ИнформацияОПользователе();
		Если Информация <> Неопределено Тогда
			Возврат Строка(Информация.Имя);
		КонецЕсли;
	Исключение
		// Если функция недоступна, возвращаем пустую строку
	КонецПопытки;
...
Функция ВыполнитьРукопожатиеСПривязкой()
	...
	Если ИдентификаторБазыДанных <> "" Тогда
		...
		// Получаем информацию о системе через вспомогательные функции
		ИмяКомпьютераЗначение = ПолучитьИмяКомпьютера();
		ИмяПользователяЗначение = ПолучитьИмяПользователя();
...
// Получение имени компьютера (клиентская версия)
&НаКлиенте
Функция ПолучитьИмяКомпьютераНаКлиенте()
	
	// Вызываем серверную функцию, так как ИнформацияОКомпьютере может быть недоступна на клиенте
	Возврат ПолучитьИмяКомпьютераНаСервере();
	
КонецФункции

// Получение имени компьютера на сервере (для вызова с клиента)
&НаСервере
Функция ПолучитьИмяКомпьютераНаСервере() Экспорт
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		// Если функция недоступна, возвращаем "Unknown"
	КонецПопытки;
...
Процедура ВыбратьКаталог(Команда)
	...
	Если ДиалогОткрытия.Выбрать() Тогда
		ИмяКомпьютераЗначение = ПолучитьИмяКомпьютераНаКлиенте();
...
&НаСервере
Функция ОбработатьПакетВыгрузки(Параметры) Экспорт
	...
	Результат.Вставить("СледующийЭтап", Этап);
	...
	Попытка
		...
		ИначеЕсли Этап = "Константы" Тогда
			РезультатПакета = ОбработатьПакетКонстант(НачальнаяПозиция, РазмерПакета);
			Результат.Вставить("НоваяПозиция", РезультатПакета.НоваяПозиция);
			Результат.Вставить("Обработано", РезультатПакета.Обработано);
			Результат.Вставить("Завершено", РезультатПакета.Завершено);
			Результат.Вставить("СледующийЭтап", РезультатПакета.СледующийЭтап);
			Результат.Вставить("ТекущийОбъект", РезультатПакета.ТекущийОбъект);
			
		ИначеЕсли Этап = "Справочники" Тогда
			РезультатПакета = ОбработатьПакетСправочников(НачальнаяПозиция, РазмерПакета);
			Результат.Вставить("НоваяПозиция", РезультатПакета.НоваяПозиция);
			Результат.Вставить("Обработано", РезультатПакета.Обработано);
			Результат.Вставить("Завершено", РезультатПакета.Завершено);
			Результат.Вставить("СледующийЭтап", РезультатПакета.СледующийЭтап);
...
&НаКлиенте
Процедура ПослеОбработкиПакета(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если Результат = Неопределено Тогда
		Сообщить("Ошибка: результат обработки пакета не получен");
	...
	Если Не Результат.Завершено И Не ТребуетсяПрервать Тогда
		...
		// Выполняем обработку на сервере и обрабатываем результат
		РезультатПакета = ОбработатьПакетВыгрузки(Параметры);
		Если РезультатПакета <> Неопределено Тогда
			ПослеОбработкиПакета(РезультатПакета);
		Иначе
			Сообщить("Ошибка: не удалось обработать пакет выгрузки");
			ЗавершитьВыгрузкуСОшибкой();
		КонецЕсли;
	Иначе
		ЗавершитьВыгрузкуНаКлиенте();
	КонецЕсли;
	
КонецПроцедуры

// Завершение выгрузки (клиентская версия)
&НаКлиенте
Процедура ЗавершитьВыгрузкуНаКлиенте()
...
// Завершение выгрузки (серверная версия)
&НаСервере
Процедура ЗавершитьВыгрузкуНаСервере()
